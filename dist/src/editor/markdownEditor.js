...
export function mountMarkdownEditor(host, {
  ...
  function clearDropTargets() {
    surface.querySelectorAll('.editor-line-text.is-drop-target').forEach(el => el.classList.remove('is-drop-target', 'is-drop-target--before', 'is-drop-target--after'));
  }
    ...
    if (display.type === 'todo') {
      ...
      handle.addEventListener('pointermove', event => {
        ...
        if (Number.isInteger(targetIndex) && canReorderTodo(doc, dragState.from, targetIndex)) {
          clearDropTargets();
          const edge = dropIndicatorEdge(dragState.from, targetIndex);
          if (edge) {
            surface.querySelector(`.editor-line-text[data-line-index="${targetIndex}"]`)?.classList.add('is-drop-target', `is-drop-target--${edge}`);
          }
  ...
  function syncGutterEditingState(index) {
    const item = gutter.querySelector(`.editor-gutter-item[data-line-index="${index}"]`);
    if (!item) return;
    item.classList.toggle('is-editing', index === activeLineIndex);
  }
